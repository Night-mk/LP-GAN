    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                p.data -= buf # weight = weight - V_t 更新参数
...
class Adam(Optimizer):
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            # print("beta1: ",learning_rate)
            # print("beta2: ",beta2)
            step_num = 2
            param_sec_group = [group['params'][i:i+step_num] for i in range(0, len(group['params']), step_num)]
            for p in param_sec_group: # p=[param1,param2]
            # for p in group['params']:
                if p[0].grad is None:
                    continue
                if p[1].grad is None:
                    continue
                # print('shape p0: ',p[0].data.shape)
                # print('shape p1: ',p[1].data.shape)
                # print('len group: ',len(group['params']))

                grad = p[0].grad+p[1].grad # 梯度还是用明文了= =
                state = self.state[p[0]] 

                if len(state) == 0:
                    state['step'] = 0 # 计算偏差校正
                    state['exp_avg'] = np.zeros(p[0].data.shape)
                    state['exp_avg_sq'] = np.zeros(p[0].data.shape)
                ...
                # 计算更新
                data_update = learning_rate*exp_avg_correction/(exp_avg_sq_correction**0.5+eps)
                ## 计算shares
                # print('p: ',p)
                data_update_1 = np.random.uniform(0,1, data_update.shape)
                data_update_2 = data_update-data_update_1
                p[0].data -= data_update_1
                p[1].data -= data_update_2
                # p.data -= learning_rate*exp_avg_correction/(exp_avg_sq_correction**0.5+eps)
